...
3. **Liveness**: No transition is dead under the initial marking $[i]$:
   $$\forall t \in T, \quad \exists M \in [N, [i]\rangle \quad \text{s.t.} \quad M \xrightarrow{t}$$
...
### 1.3 Alignment Cost Formulation
...
- **Synchronous Move**: $(t, a)$ where $\lambda(t) = a$.
- **Move on Log**: $(\gg, a)$ representing a process deviation where activity $a$ occurs in the log but is not replayed in the model.
- **Move on Model**: $(t, \gg)$ representing a deviation where transition $t$ is fired in the model but not observed in the log.

Let the cost function $c$ associate a non-negative cost with each move:
- $c(t, a) = 0$ if $\lambda(t) = a$.
- $c(\gg, a) = c_{\text{log}}(a) > 0$.
- $c(t, \gg) = c_{\text{model}}(t) > 0$ if $t$ is visible, and $c(t, \gg) = 0$ if $\lambda(t) = \tau$.

For a trace $\sigma \in \Sigma^*$ and a WF-net $N$, a valid alignment $\gamma$ is a sequence of moves whose projection onto the log yields $\sigma$, and whose projection onto the model yields a valid firing sequence $\tau \in T^*$ from $[i]$ to $[o]$ (i.e., $[i] [\tau\rangle [o]$).

The **Optimal Alignment** $\gamma^*$ minimizes the total deviation cost:
$$\gamma^* = \operatorname{argmin}_{\gamma \in \text{Align}(\sigma, N)} \sum_{m \in \gamma} c(m)$$
...
Declare models specify templates mapped to Linear Temporal Logic over Finite Traces (LTLf). Let a trace be $\sigma = \langle e_1, e_2, \dots, e_n \rangle$ of length $n$. For an index $i \in \{1, \dots, n\}$:
...
- $\sigma, i \models \mathbf{X} \varphi \iff i < n \text{ and } \sigma, i+1 \models \varphi$ (Next)
- $\sigma, i \models \mathbf{F} \varphi \iff \exists j \in \{i, \dots, n\} \text{ s.t. } \sigma, j \models \varphi$ (Eventually)
- $\sigma, i \models \mathbf{G} \varphi \iff \forall j \in \{i, \dots, n\}, \sigma, j \models \varphi$ (Always)
- $\sigma, i \models \varphi \mathbf{U} \psi \iff \exists j \in \{i, \dots, n\} \text{ s.t. } \sigma, j \models \psi \text{ and } \forall k \in [i, j-1], \sigma, k \models \varphi$ (Until)
- The weak until operator $\mathbf{W}$ is defined as $\varphi \mathbf{W} \psi \equiv \varphi \mathbf{U} \psi \lor \mathbf{G} \varphi$.
...
| **Precedence(A, B)** | $\neg B \mathbf{W} A$ | $B$ must not occur unless preceded by $A$. |
| **Succession(A, B)** | $\mathbf{G}(A \implies \mathbf{F} B) \land (\neg B \mathbf{W} A)$ | $A$ occurs if and only if $B$ occurs afterward. |
...
$$\sigma^* = \operatorname{argmin}_{\sigma' \models \mathcal{D}} \operatorname{dist}(\sigma, \sigma')$$
$$\text{Cost}(\sigma, \mathcal{D}) = \operatorname{dist}(\sigma, \sigma^*)$$
...
$$n - i = N_{\text{log}} + N_{\text{sync}}$$

The model transitions in $\gamma_{\text{tail}}$ must transition the marking from $s$ to $s_f$. The set of transitions fired in the model is the union of transitions fired during moves-on-model and synchronous moves. The minimum cost to transition the model from $s$ to $s_f$ using only moves-on-model is $d_M(s)$. 

Since a synchronous move $(t, a)$ fires transition $t$ but incurs a cost of 0, each synchronous move reduces the required model-move-only cost by at most the cost of that transition, which is bounded by $\theta$:
$$\sum_{t \in T_{\text{model}}} c(t, \gg) \ge d_M(s) - \sum_{t \in T_{\text{sync}}} c(t, \gg) \ge d_M(s) - N_{\text{sync}} \cdot \theta$$

The total actual cost of the alignment tail $\gamma_{\text{tail}}$ is:
$$\text{Cost}(\gamma_{\text{tail}}) = \sum_{j=1}^{N_{\text{log}}} c(\gg, a'_j) + \sum_{k=1}^{N_{\text{model}}} c(t'_k, \gg)$$

Since move-on-log costs are non-negative ($c(\gg, a'_j) \ge 0$), we have:
$$\text{Cost}(\gamma_{\text{tail}}) \ge \sum_{k=1}^{N_{\text{model}}} c(t'_k, \gg) \ge d_M(s) - N_{\text{sync}} \cdot \theta$$

Using $N_{\text{sync}} \le n - i$, we obtain:
$$\text{Cost}(\gamma_{\text{tail}}) \ge d_M(s) - (n - i) \cdot \theta$$

Furthermore, because all move costs are non-negative, the total cost of any alignment tail must be non-negative:
$$\text{Cost}(\gamma_{\text{tail}}) \ge 0$$

Combining these two inequalities, we get:
$$\text{Cost}(\gamma_{\text{tail}}) \ge \max\left(0, d_M(s) - (n - i) \cdot \theta\right) = h(s, i)$$
...
- **$(\implies)$ Direction**: Suppose $\operatorname{Cost}_{\text{opt}}(\sigma, N) = 0$. Since all non-synchronous moves (moves-on-log and visible moves-on-model) have strictly positive costs, a cost of zero implies that the optimal alignment contains only synchronous moves and silent transitions. The sequence of activities in the synchronous moves matches $\sigma$ exactly, and the corresponding transition sequence $\tau$ satisfies $[i] [\tau\rangle [o]$ in $N$. Therefore, $\sigma \in \mathcal{L}(N)$.
- **$(\impliedby)$ Direction**: Suppose $\sigma \in \mathcal{L}(N)$. Then there exists a transition sequence $\tau$ such that $[i] [\tau\rangle [o]$ and the projection of $\tau$ onto the visible activities is exactly $\sigma$. We can construct an alignment consisting of synchronous moves for all visible transitions in $\tau$ and silent model moves for all silent transitions in $\tau$. Since synchronous moves and silent transitions have a cost of 0, the cost of this alignment is 0. Since the cost is bounded below by 0, the optimal alignment cost must be 0.
...
Let $(S_1, \to_1)$ and $(S_2, \to_2)$ be the transition systems of $N_1$ and $N_2$. A relation $\mathcal{R} \subseteq S_1 \times S_2$ is a **weak bisimulation** if for all $(s_1, s_2) \in \mathcal{R}$:
1. If $s_1 \xrightarrow{a} s'_1$ for some $a \in \Sigma$, then there exists $s'_2 \in S_2$ such that $s_2 \xrightarrow{a}_{\text{weak}} s'_2$ and $(s'_1, s'_2) \in \mathcal{R}$.
2. If $s_2 \xrightarrow{a} s'_2$ for some $a \in \Sigma$, then there exists $s'_1 \in S_1$ such that $s_1 \xrightarrow{a}_{\text{weak}} s'_1$ and $(s'_1, s'_2) \in \mathcal{R}$.
where $\xrightarrow{a}_{\text{weak}} = \xrightarrow{\tau}^* \xrightarrow{a} \xrightarrow{\tau}^*$ represents transition steps allowing silent transitions.
